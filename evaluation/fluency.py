    ...
    def convert_inputs_to_sentences(self, x):
        ...
        for i, w in enumerate(x):
            ...
            if i - split_ids[-1] > self.sentence_length:
                # print('i = '+str(i))
                # print('split_ids[-1] = '+str(split_ids[-1]))
...
class MaskedBert(ModelMixin):
    ...
    def score(self, x, temperature=1.0, batch_size=50, verbose=False, *args, **kwargs):
        self.model.eval()
...
# sentences = []
# with open('shakespeare-data/shakespeare_test_bart_base_ver1.txt', 'r') as fr:
#     for i, line in enumerate(fr):
#         if (i%2==1) and (len(line)<110):
#             sentences.append(line.split('\t')[1])
# print(calc_fluency(sentences))
