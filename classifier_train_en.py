...
# 第三部分，整理数据集
train_dataset = XYDataset('trump-classifier/classifier-train.txt')
dev_dataset = XYDataset('trump-classifier/classifier-dev.txt')
batch_size = 32
patience = 8
...
torch.save(checkpoint,'saved_models/trump-classifier/checkpoint.pkl')
    ...
    for batch in tqdm(train_dataloder, desc="Training"):
        ...
        if train_step_total % num_checkpoint_steps == 0 and epoch >= 3:
            ...
            if dev_loss_average < dev_loss_best or dev_loss_best==0:
                ...
                with open('saved_models/trump-classifier/loss.txt', 'w') as fw:
